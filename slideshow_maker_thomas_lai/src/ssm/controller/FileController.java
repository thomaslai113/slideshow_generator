...
public class FileController {
    ...
    public void handleNewSlideShowRequest() {
        try {
            ...
            if (continueToMakeNew) {
                ...
                if(ui.getCanc() == false)
                {
                ui.changeTitle.setDisable(false);
		...
		slideShow.reset();
                ...
                PropertiesManager ps = PropertiesManager.getPropertiesManager();
                ui.promptError(ps.getProperty(LanguagePropertyType.SLIDEMSG));
                }
            }
        } catch (IOException ioe) {
            ErrorHandler eH = ui.getErrorHandler();
            PropertiesManager ps = PropertiesManager.getPropertiesManager();
            eH.processErrors(LanguagePropertyType.ERROR_DATA_FILE_LOADING,LanguagePropertyType.ERRORDATA , ioe.getMessage());
        }
    }
    ...
    public boolean handleSaveSlideShowRequest() {
	    ...
	    errorHandler.processErrors(LanguagePropertyType.ERROR_DATA_FILE_LOADING, LanguagePropertyType.ERRORDATA , ioe.getMessage());
	    return false;
        }
    }
    ...
    private boolean promptToSave() throws IOException {
        ...
        int saveWork = ui.justAsk(); // @todo change this to prompt
    ...
    private void promptToOpen() {
		...
		SlideShowModel slideShowToLoad = ui.getSlideShow();
                ...
                slideShowToLoad.reloadAnyway();
                saved = true;
                ui.updateToolbarControls(saved);
            } catch (Exception e) {
                ErrorHandler eH = ui.getErrorHandler();
                 eH.processErrors(LanguagePropertyType.ERROR_DATA_FILE_LOADING, LanguagePropertyType.ERRORDATA , e.getMessage());
            }
        }
    }
